...
public class KGGraphRepository implements KGGraphDao {
    ...
    public void updateKGDomain(Integer domainId, String newDomain) {
        try {
            String cypherSql = String.format(
                    "match (n:`%s`) set n.name='%s'", domainId,newDomain);
            Neo4jUtil.runCypherSql(cypherSql);
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    public void batchInsertByCsv(String domain, String csvUrl, int isCreateIndex) {
        ...
        addIndexCypher = "CREATE INDEX IF NOT EXISTS FOR (n:`" + domain + "`) ON (n.name);";
        loadNodeCypher1 = "CALL { " +
                "    LOAD CSV FROM '" + csvUrl + "' AS line " +
                "    MERGE (:`" + domain + "` {name:line[0]}) " +
                "} IN TRANSACTIONS OF 500 ROWS;";
        loadNodeCypher2 = "CALL { " +
                "    LOAD CSV FROM '" + csvUrl + "' AS line " +
                "    MERGE (:`" + domain + "` {name:line[1]}) " +
                "} IN TRANSACTIONS OF 500 ROWS;";
        // 拼接生产关系导入cypher
        ...
        loadRelCypher = "CALL { " +
                "    LOAD CSV FROM '" + csvUrl + "' AS line " +
                "    MATCH (m:`" + domain + "`),(n:`" + domain + "`) WHERE m.name=line[0] AND n.name=line[1] " +
                "    MERGE (m)-[r:" + type + "]->(n) " +
                "    SET r.name=line[2] " +
                "} IN TRANSACTIONS OF 500 ROWS;";
