...
import com.warmer.web.config.ConversionConfig;
...
public class KGBuilderController extends BaseController {
        ...
        try {
            if (!StringUtil.isBlank(domain)) {
                ...
                if (domainItem.size() > 0) {
                    ...
                    String label= domain;//String.format("%s_%s",domain, IdUtil.nanoId(6));
    ...
    public R<String> updateDomain(Integer domainId, String newDomain) {
        try {
            if (!StringUtil.isBlank(newDomain)) {
                List<KgDomain> domainItem = kgManagerService.getDomainByName(newDomain);
                if (domainItem.size() > 0) {
                    return R.create(ReturnStatus.Error, "领域已存在");
                } else {
                    kgManagerService.updateDomain(domainId, newDomain); // 更新 MySQL 中的领域信息
                    kgGraphService.updateKGDomain(domainId, newDomain); // 更新图数据中的领域信息
                    kgManagerService.deleteDomain(domainId);
//                    kgGraphService.deleteKGDomain(domain);
                    return R.success("领域更新成功");
                }
            } else {
...
//        System.out.println("开始导出图谱");
        Map<String, Object> res = new HashMap<>();
        String label = request.getParameter("domain");
        String filePath = "D:\\kgmanager\\exportGraph\\"; // 确保这个路径是Web可访问的
        ...
        try {
            ...
            csvWriter.close();
            res.put("code", 200);
            res.put("fileName", "/exportGraph/" + fileName); // 提供一个可通过HTTP访问的下载路径
...
//        String filePath = "D:\\kgmanager\\exportGraph\\";
//        String fileName = UUID.randomUUID() + ".csv";
//        String fileUrl = filePath + fileName;
//        String cypher = String.format(
//                "MATCH (n:`%s`) -[r]->(m:`%s`) return n.name as source,m.name as target,r.name as relation", label, label);
...
////            String csvUrl = fileUrl;
//            res.put("code", 200);
//
//            res.put("fileName", csvUrl);
//            res.put("message", "success!");
//            System.out.println("路径："+csvUrl);
//            return res;
//        } catch (Exception e) {
//            e.printStackTrace();
//        }
...
//    }
    ...
    public R<String> saveNodeImage(@RequestBody Map<String, Object> params) {
        try {
            ...
            //测试
            System.out.println("saveNodeImage:");
